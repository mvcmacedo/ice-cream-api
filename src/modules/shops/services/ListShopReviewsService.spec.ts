import 'reflect-metadata';
import nock from 'nock';

import FakeCacheProvider from '@shared/providers/CacheProvider/fakes/FakeCacheProvider';
import HttpClientProvider from '@shared/providers/HttpClientProvider/implementations/AxiosHttpClientProvider';

import AppError from '@shared/errors/AppError';
import ListShopReviews from './ListShopReviewsService';

describe('ListShopReviews', () => {
  const fakeCacheProvider = new FakeCacheProvider();
  const httpClientProvider = new HttpClientProvider(); // using nock library instead a fake implementation

  const shopId = 'test';
  const yelpUrl = 'https://test.com.br';

  beforeAll(() => {
    process.env.YELP_URI = yelpUrl;
    process.env.YELP_APP_KEY = 'test';
  });

  afterEach(async () => {
    nock.cleanAll();
    await fakeCacheProvider.invalidateAll();
  });

  it('should list shop reviews (calling from yelp api)', async () => {
    const getShop = new ListShopReviews(httpClientProvider, fakeCacheProvider);

    const fakeResponse = {
      reviews: [
        {
          id: 'test',
          name: 'test name',
        },
      ],
    };

    nock(yelpUrl).get(`/businesses/${shopId}/reviews`).reply(200, fakeResponse);

    const shops = await getShop.execute({ shopId });

    expect(shops).toBeInstanceOf(Array);
    expect(shops).toHaveLength(1);
  });

  it('should list shop reviews (calling from cache)', async () => {
    const getShop = new ListShopReviews(httpClientProvider, fakeCacheProvider);

    const fakeResponse = {
      reviews: [
        {
          id: 'test',
          name: 'test name',
        },
      ],
    };

    nock(yelpUrl)
      .get(`/businesses/${shopId}/reviews`)
      .once()
      .reply(200, fakeResponse);

    await getShop.execute({ shopId });
    const shops = await getShop.execute({ shopId });

    expect(shops).toBeInstanceOf(Array);
    expect(shops).toHaveLength(1);
  });

  it('should NOT list shop reviews (yelp api error)', async () => {
    const message = 'test error';
    const getShop = new ListShopReviews(httpClientProvider, fakeCacheProvider);

    nock(yelpUrl).get(`/businesses/${shopId}/reviews`).replyWithError({
      message,
    });

    const shop = getShop.execute({ shopId });

    expect(shop).rejects.toBeInstanceOf(AppError);
    expect(shop).rejects.toHaveProperty('message', message);
  });
});
